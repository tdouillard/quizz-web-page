...
let quizzCore, quizzRender, quizzProgress;
...
function renderCurrentQuestion() {
    ...
    const currentAnswer = quizzProgress.getAnswer(quizzCore.getCurrentQuestionIndex());
    
    quizzRender.renderQuestionIntoContainer(question, currentAnswer, quizzCore.getCurrentQuestionIndex());
}
...
function showQuizResults() {
    const results = quizzProgress.exportData();
    const container = document.getElementById('question-container');
    ...
    const resultsHTML = `
        <div class="quiz-results">
            ...
            <div class="results-summary">
                <p><strong>Time taken:</strong> ${quizzProgress.getFormattedElapsedTime()}</p>
                <p><strong>Questions answered:</strong> ${results.answeredCount} / ${results.totalQuestions}</p>
                <p><strong>Completion:</strong> ${results.progressPercentage}%</p>
            </div>
            <div class="results-details">
                <h3>Your Answers:</h3>
                ${generateAnswerSummary()}
            </div>
            <button onclick="restartQuiz()" style="margin-top: 20px;">Start Over</button>
        </div>
    `;
...
function generateAnswerSummary() {
    ...
    for (let i = 0; i < quizzCore.getTotalQuestions(); i++) {
        const question = quizzCore.getQuestion(i);
        const answer = quizzProgress.getAnswer(i);
        
        summaryHTML += `
            <div class="answer-summary">
                <strong>Q${i + 1}:</strong> ${question.name}<br>
                <strong>Your answer:</strong> ${formatAnswer(answer, question.type)}
            </div>
        `;
    }
